    ...
    return new Promise(async (resolve, reject) => {
      try {
        preCallout(opts).then((opts) => {
          const token = opts.token && opts.token.value ? opts.token : null;
...
async function preCallout(opts) {
  ...
  try {
    if (token) {
      if (tokenExpired(token) && opts.token.refresh) {
        opts.token.value = await refreshToken(opts);
        opts.token.refreshToken = opts.token.value;
      }
    }
