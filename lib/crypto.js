...
exports.decryptStream = (key, header, opts) => {
    ...
    function transform(chunk, encoding, callback) {
      if (toSkip == 0) {
        message = module.exports.secretStreamPull(chunk, state);
      } else if (toSkip > chunk.length) {
        toSkip -= chunk.length;
      } else {
        if (toSkip !== chunk.length) {
          message = module.exports.secretStreamPull(chunk.slice(toSkip), state);
        }
        toSkip = 0;
      }
