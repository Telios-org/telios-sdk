...
class Drive extends EventEmitter {
  ...
  async ready() {
    await this._initCorestore();

    if(this.seed) {
      try {
        const lastSeq = await this.coredb.get('lastSeq');
  ...
  _syncDiffs(currentVer) {
    const ds = this.db.createDiffStream(currentVer);
    ds.on('data', async diff => {
    ...
    ds.on('end', () => {
      setTimeout(async () => {
        const lastVer = await this.coredb.get('lastSeq');
  ...
  _syncRemoteUpdates() {
    const hs = this.db.createHistoryStream({ gt: this._lastSeq, live: true })

    hs.on('data', async data => {
      this.coredb.put('lastSeq', { seq: this.db.version });
      if(!this._diffSyncInProgress || this._lastSeq === 0) {
        
        // Make updates here since we're all caught up
        if(data.type === 'put' && data.value.name) {
          await this.writeFile({ driveKey: data.value.core, corePath: data.value.path });
   ...
   */
  async writeFile({ driveKey, corePath, readStream, encrypted }) {

    // Sync remote drive and return
    if(driveKey) {
      return this._initCore(corePath, { driveKey });
    }

    return new Promise(async (resolve, reject) => {
      const pathSeg = corePath.split('/');
      ...
      const core = await this._initCore(corePath, { replicate: false });
      const writeStream = core.createWriteStream('/' + fullFile);

      if (encrypted) {
        const { key, header, file } = await Crypto.encryptStream(readStream, writeStream);

        await this.db.put(corePath, {
          hash: file.hash
        });
  ...
  async readFile(filePathOrHash, opts) {
    return new Promise(async (resolve, reject) => {
      ...
      const core = this.corestore[res.value.path];

      // Get file stream from core
      const encStream = core.createReadStream(`${res.value.name}.${res.value.mimetype}`);
  ...
  async _destroyStorage(corePath) {
    const core = this.corestore[corePath];

    if(core) {
      await core.close()
      delete this.corestore[corePath];

      rimraf.sync(path.join(`${this.drivePath}/${corePath}`));
    }
  }

  async _initCorestore() {
    const asyncArr = [];
    this.coredb = level(path.join(this.metaPath, '/corestore'), { valueEncoding: 'json' });

    return new Promise((resolve, reject) => {
      this.coredb.createReadStream()
        .on('data', (data) => {
          if(data.value.coretype === 'hyperdrive') {
            asyncArr.push(this._initCore(data.key));
          }
        })
        .on('error', (err) => {
          reject(err);
  ...
  async _initCore(corePath, opts) {
    let core;

    return new Promise(async (resolve, reject) => {
      this.coredb.get(corePath, (err, value) => {

        if(value && this.corestore[corePath]) {
          return resolve(this.corestore[corePath]);
        }
      
        core = Hyperdrive(path.join(this.drivePath, corePath), opts.driveKey, { sparse: false, sparseMetadata: false });
    
        core.on('ready', () => {
          const key = core.key.toString('hex');
          
          if(!value) {
            this.coredb.put(corePath, { 
              coretype: 'hyperdrive',
              key,
