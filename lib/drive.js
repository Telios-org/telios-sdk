...
class Drive extends EventEmitter {
  constructor(drivePath, publicKey, { keyPair, writable }) {
    ...
    this.hyperstore = {};
    this.corestore = new Corestore(path.join(drivePath, './corestore'));
    // Local DB feed
    this.storeFeed = Hypercore(path.join(drivePath, './.meta/hyperstore'));
    // Local DB
    this.hyperdb = null;
  ...
  async ready() {
    ...
    // Create default hypercore for bootstrapping additional cores
    ...
    server.on('file-request', async (data, peer) => {
      console.log(`File request ${JSON.stringify(data.files)} | from peer ${data.peerPubKey}`);
      const fileRequests = [];
      ...
      diffStream.on('data', async diff => {
        if(currentVer > 0) {
          ...
          if(diff.left && diff.left.value && diff.left.value.name) {
            await this.writeFile(diff.left.value.path, { feedKey: diff.left.value.key });
  ...
  _syncRemoteUpdates() {
    ...
    historyStream.on('data', async data => {
      ...
      if(!this._diffSyncInProgress || this._lastSeq === 0) {
        this.hyperdb.put('lastSeq', { version: data.seq });
   ...
   */
  async writeFile(filePath, { feedKey, readStream, encrypted }) {
    if(filePath[0] === '/') {
  ...
  async _initHyperstore() {
    ...
    this.hyperdb = new Hypertrie(null, {
      feed: this.storeFeed,
      keyEncoding: 'utf-8',
      valueEncoding: 'json'
    });
    ...
    return new Promise((resolve, reject) => {
      this.storeFeed.on('ready', async () => {
        
        await _setLastDBVersion();
        
        this.hyperdb.createReadStream()
          .on('data', (data) => {
            // TODO: Use Hypertrie prefix to iterate over hypercores
    ...
    function _setLastDBVersion() {
      return new Promise((resolve, reject) => {
          self.hyperdb.get('lastSeq', (err, item) => {
            if(err) return reject(err);
            
            if(item) {
              this._lasSeq = item.value.version;
            }
            resolve();
          });
      });
  ...
  async close() {
    ...
    this.hyperdb.feed.close(() => {
      this.db.feed.close(async () => {
