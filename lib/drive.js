...
class Drive extends EventEmitter {
  constructor(drivePath, publicKey, { keyPair, ignore, ephemeral, writable }) {
    ...
    // P2P DB
    this.db = null;
    // Local conifg store for slave drives
    this.slaveDB = null;
    // Drive owner keypair
      ...
      server.on('file-request', async (data, peer) => {
        ...
        for(let file of data.files) {
          fileRequests.push(new Promise(async (resolve, reject) => {
            ...
            const rs = fs.createReadStream(`${this.drivePath}/${_file.value.filename}`);
   ...
   */
  async addPeer(diffKey, access) {
    if(access && access.indexOf('write') > -1) {
      if(!this.writable) {
        try {
          await this._updateSlaveWriters([diffKey]);
        } catch(err) {
          throw err;
        }
      } else {
        this._updateWriters([diffKey]);
      }
    }
          ...
          if(peer.publicKey.toString('hex') !== p.publicKey && !connected) {
            ...
            for(let file of files) {
              fileRequests.push(new Promise((res, rej) => {
                const writeStream = fs.createWriteStream(file.dest);
                const stream = peer.receiveStream(file.hash);

                pump(stream, writeStream, async (err) => {
                  if(err) return rej(err);
                  
                  eventEmitter.emit('file-download', { 
                    path: file.dest, 
                    hash: file.hash, 
                    source: 'remote' 
                  });
  ...
  _watchLocalDrive() {
    ...
    this._watcher
      ...
      .on('unlink', async (filename) => {
        ...
        this.emit('file-unlink', { filePath: path.join(__dirname, file.value.path), size: file.value.size, source: 'local' });
      });
  }
        ...
        plex.on('connection', peer => {
          ...
          if(peer.publicKey.toString('hex') !== peerKey.toString('hex')) {
            if(!this._incomingFiles[file.filename]) {
              ...
              fs.mkdirSync(`${this.drivePath}${file.path}`, { recursive: true });

              const filePath = `${this.drivePath}${file.path}${file.filename}`;
              ...
              pump(stream, ws, (err) => {
                ...
                //TODO: Validate the local file hash matches the remote drive's hash
                  //TODO: If they don't match, then remove the file
                  // fs.rmdirSync(`${this.drivePath}${file.path}`, { recursive: true });
