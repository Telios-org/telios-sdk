...
class Drive extends Auth {
  constructor(drivePath, publicKey, { keyPair, ignore, seed, live, watch, peers, access, slave }) {
    super();
    ...
    this.peers = peers; // Connected peers (should be peers acting as servers)
  ...
  async ready() {
    ...
    if(this._create && this.publicKey) {
       ...
       */
      if(this.slave) {
        this.slaveDB = level(path.join(this.metaPath, '/.drive/access'), { valueEncoding: 'json' });
    ...
    if(this.seed) {
      ...
      this._connections.push(server.hyperswarm);
    }
    ...
    if (this.live) {
      ...
      this._connections.push(replicate.hyperswarm);
      ...
      this._connections.push(client.hyperswarm);
    }
  ...
  _watchLocalDrive() {
    ...
    this._watcher
      ...
      .on('change', async (filename) => {
        if(this._incomingFiles[filename]) return;
  ...
  _watchRemoteDrive() {
    this._stream = this.db.createHistoryStream({ live: true, gte: -1 });
      ...
      if (data.value && data.value.filename && this.readyToWatch) {
        try {
          ...
          if(needsUpdate) {
            ...
            // this._connections.push(plex.publicKey.toString('hex'));
            ...
            //   if(this._connections.indexOf(peer.publicKey.toString('hex')) === -1) {
  ...
  async _fileNeedsUpdate(file) {
    return new Promise((resolve, reject) => {
      ...
      checksumFile('sha256', filePath)
        .then(async hash => {
          console.log(`local hash ${hash} : inc hash ${file.hash}`);
          if(hash !== file.hash) {
            return resolve(true);
          }
  ...
  async close() {
    for(conn of this._connections) {
      this._connections[conn].close();
    }
    ...
    if(this._diffHyperbee.feed.opened) {
      this._diffHyperbee.feed.close();
    }
  }
}
