...
class Drive extends Auth {
  constructor(drivePath, publicKey, { keyPair, ignore, seed, watch, slave }) {
    super();

    this.db = null; // P2P DB
    this.slaveDB = null; // Local conifg store for slave drives
    this.keyPair = keyPair; // Drive owner keypair
    this.seed = !seed ? true : seed; // Seed this drive to peers when ready
    ...
    this.watch = !watch ? true : watch; // If true watch for updates to local drive and write those updates to the DB
  ...
  async ready() {
    ...
    /**
     * Set watch to false on drives that just seed content and don't intend to write or
    ...
    this._watcher
      .on('add', async (filename) => {
        ...
        if(!stats.isDirectory()) {
          ...
          this.emit('file-upload', { filename, path: fullPath, hash });
        }
      })
  ...
  _watchRemoteDrive() {
    ...
    hs.on('data', async data => {
      ...
      if(data.key === '__access' && !this.watch) {
