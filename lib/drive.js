    ...
    this._watcher
      .on('add', async (filename) => {
        ...
        if(!stats.isDirectory()) {
          ...
          const fileHash = await this.db.get(file.value.hash);
      ...
      if (data.value && data.value.filename && this.readyToWatch) {
        ...
        try {
          ...
          if(needsUpdate && !this._incomingFiles[data.value.filename]) {
            ...
            let event = 'file-update';

            // If incoming file doesn't exist then emit file add event
            if(!fs.existsSync(path.join(this.drivePath, `/${data.value.filename}`))) {
              event = 'file-add';
            }

            // console.log(this.peers.indexOf(data.value.updatedBy) > -1);
            ...
            hostPeerKey = Buffer.from(data.value.updatedBy, 'hex');
            plex.join(hostPeerKey, { announce: false, lookup: true });
            // if(this.peers.indexOf(data.value.updatedBy) > -1) {
            //   console.log(this.peers)
            //   hostPeerKey = Buffer.from(data.value.updatedBy, 'hex');
            //   plex.join(hostPeerKey, { announce: false, lookup: true });
            // } else {
              ...
              plex.on('connection', peer => {
                if(peer.publicKey.toString('hex') !== hostPeerKey.toString('hex')) {
                  if(!this._incomingFiles[data.value.filename]) {
                    ...
                    const filePath = `${this.drivePath}/${data.value.filename}`;
                    ...
                    pump(stream, ws, (err) => {
                      ...
                      setTimeout(() => {
                        ...
                        plex.destroy();
                        console.log(`File ${data.value.filename} Retrieved!`)
                        console.log(' ');
                        this._busy = false;
