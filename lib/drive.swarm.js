    ...
    this.hyperswarm.on('connection', async (socket, info) => {
      if (this.db) {
        ...
        if(this.announce) {
          ...
          // memStream.on('data', async data => {
          //   console.log(`Request for file ${data.toString('utf-8')}`);

          //   const file = await this.db.get(data.toString('utf-8'));
          ...
          plex.on('connection', peer => {
            ...
            stream.on('data', data => {
              console.log(`Request for file ${data.toString('utf-8')}`);
              this.emit('file-request', data.toString('utf-8'), peer);
            });
          });
        }
        ...
        if(this.lookup) {
          socket.once('data', data => {
            ...
            if(peer.publicKey !== this.keyPair.publicKey) {
