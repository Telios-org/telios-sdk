...
class Hyperbee {
  ...
  async addPeer(key) {
    return this.db.addPeer(key);
  }

  async put(key, value) {  
    if (this.pubKey && this.privKey) {
      key = Crypto.encryptSBMessage(key, this.pubKey, this.privKey).toString('hex');
      value.data = Crypto.encryptSBMessage(JSON.stringify(value.data), this.pubKey, this.privKey).toString('hex');
    }
  ...
  async get(key) {
    if (this.pubKey && this.privKey) {
      ...
      const decoded = Crypto.decryptSBMessage(value.data, this.pubKey, this.privKey);
