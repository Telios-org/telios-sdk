...
class Mailbox extends Client {
  ...
  async getNewMail(privKey, sbpkey) {
    ...
    for (let i = 0; i < newMail.length; i++) {
      promises.push(this._decryptMail(newMail[i], privKey, sbpkey));
    }
  ...
  async _decryptMail(sealedMeta, privKey, sbpkey) {
    ...
    if (env === 'test') {
      opts.storage = '../tests/drive';
    }
    console.log(opts);
    const hyperdrive = new Hyperdrive(opts);
    await hyperdrive.connect();
    console.log(await hyperdrive.drive.readdir('/'));
  ...
  async send(email, opts) {
    ...
    if (extRecipients) {
      ...
      */
      const mailMeta = [];
      const { location, key, npub } = await this._encryptMail(email, opts);
