...
class Mailbox extends Client {
  ...
  async getNewMail(privKey, sbpkey) {
    ...
    for(let i = 0; i < newMail.length; i++) {
      promises.push(await this._decryptMail(newMail[i], privKey, sbpkey));
    }
  ...
  async _storeSentMail(driveOpts, path, msg) {
  ...
  static async registerMailEventListener(opts) {
    const hypercore = new Hypercore(opts);
    const feed = await hypercore.createCore();
    return feed;
  }
}
