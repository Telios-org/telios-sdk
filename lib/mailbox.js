...
class Mailbox extends Client {
  ...
  async getNewMail(privKey, sbpkey) {
    ...
    for (let i = 0; i < newMail.length; i++) {
      metaList.push(this._decryptMailMeta(newMail[i], privKey, sbpkey));
    }
  ...
  async send(email, opts) {
    ...
    if (extRecipients) {
      ...
      */
      ...
      const uuid = uuidv4();
      const location = `${opts.filePath}${uuid}`;
      const writeStream = fs.createWriteStream(path.join(opts.drive.drivePath, location));
      const stream = new MemoryStream();

      stream.end(JSON.stringify(email));

      const { file, key, header } = await this._encryptStream(stream, writeStream, opts);
  ...
  _decryptMailMeta(sealedMeta, privKey, sbpkey) {
    let email = "";
  ...
  async _encryptStream(readStream, writeStream, opts) {
    ...
    return new Promise((resolve, reject) => {
      ...
      opts.drive.on('file-upload', file => {
