...
test("Account - Make Keypairs", async (t) => {
  ...
  t.ok(
    secretBoxKeypair.privateKey,
    `Secret box private key: ${secretBoxKeypair.privateKey}`
  );
  t.ok(
    secretBoxKeypair.publicKey,
    `Secret box public key: ${secretBoxKeypair.publicKey}`
  );

  t.ok(
    signingKeypair.privateKey,
    `Signing private key: ${signingKeypair.privateKey}`
  );
  t.ok(
    signingKeypair.publicKey,
    `Signing public key: ${signingKeypair.publicKey}`
  );
  ...
  try {
    const opts = {
      account: {
        ...
        account_drive_key: conf.ALICE_DRIVE_KEY,
